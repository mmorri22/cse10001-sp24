...
int main( const int argc, const char* argv[] ){
    ...
    while( lib_file != NULL && iter < num_entries ){
        ...
        if( fscanf(lib_entry, "%s", the_lib[iter].title )){}
        if( fscanf(lib_entry, "%s", the_lib[iter].course_dept )){}
        if( fscanf(lib_entry, "%u", &the_lib[iter].course_num )){}
        if( fscanf(lib_entry, "%u", &the_lib[iter].num_books )){}
